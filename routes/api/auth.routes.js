    ...
    try {
        ...
        if (!errors.isEmpty()) {
            return res.status(400).json({
                errors: errors.array(), 
                message: "Invalid reg data during register"});
        }
        ...
        res.status(500).json({ message: "Something went wrong, try again", error: e.message });
...
router.post(
    ...
    try {
        ...
        if (!errors) {
            return res.status(400).json({
                errors: errors.array(), 
                message: "Invalid reg data during logging in"});
        }
